...
class AuthenticationTests(TestCase):
    ...
    def test_signup(self):
        ...
        # Positive Testing
        data = {'username': 'adityasutar', 'password': 'aditya1234', 'first_name': "Aditya", 'last_name': "Sutar",
                'email': 'adityasutar12@gmail.com'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(response.data['status'])
        self.assertEqual(response.data['message'], 'User Created Successfully')
        self.assertEqual(response.data['data']['username'], 'adityasutar')
        self.assertEqual(response.data['data']['first_name'], 'Aditya')
        self.assertEqual(response.data['data']['last_name'], 'Sutar')
        self.assertEqual(response.data['data']['email'], 'adityasutar12@gmail.com')

        # Negative testing with Incomplete Data
        incomplete_data = {'username': 'adityasutar', 'password': 'aditya1234'}
    ...
    def test_login(self):
        url = '/api/login'
        user = User.objects.create_user(username='adityasutar', password='aditya1234')
        ...
        invalid_data = {'username': 'adityapatil', 'password': 'aditya333'}
        response = self.client.post(url, invalid_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertFalse(response.data['status'])
        self.assertIn('Invalid Login Credential', response.data['message'])

        Wrong_data = {'username1': 'adityasutar', 'secrete': 'aditya333'}
...
class VendorAPITests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(username='adityasutar', password='aditya1234')
        self.client.force_authenticate(user=self.user)

    def test_vendor(self):
        # url_login = '/api/login'
        # user = User.objects.create_user(username='adityasutar', password='aditya1234')
        # login_data = {'username': 'adityasutar', 'password': 'aditya1234'}
